package com.rising.insta.utils;
...
public class NaverSensV2 {
    ...
    @Value("${sensToPhoneNumber}")
    private String toPhoneNumber;


	@SuppressWarnings("unchecked")
	public void send_msg(String tel, String rand) throws BaseException {
		...
		String content = "RisingTest 인스타그램(iOS) 본인인증 번호입니다. [" + rand + "]";

		toJson.put("content", content); // 난수와 함께 전송
		toJson.put("to", tel);
		toArr.add(toJson);
		...
		bodyJson.put("from", toPhoneNumber); // 발신번호 * 사전에 인증/등록된 번호만 사용할 수 있습니다.
		bodyJson.put("messages", toArr);

		//bodyJson.put("subject", "제목");
		...
		try {
			...
			throw new BaseException(AUTH_PHONE_INVALID_SEND_MESSAGE);
		}
	}
